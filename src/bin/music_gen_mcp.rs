        ...
        "tools": [
            {
                ...
                "description": "Liste les modèles musicaux et d'effets sonores installés.",
                "inputSchema": { "type": "object", "properties": {} }
            },
            {
                "name": "generate_music",
                "description": "Compose un morceau de musique WAV ou un effet sonore à partir d'une description.",
                "inputSchema": {
                    "type": "object",
                    "properties": {
                        "prompt": { "type": "string", "description": "Description musicale (instruments, ambiance, tempo, mélodie)" },
                        "genre": { "type": "string", "description": "Genre musical (ex: Synthwave, Lo-Fi, Orchestral, Jazz)" },
                        "duration_seconds": { "type": "integer", "description": "Durée en secondes (ex: 10, 30)" }
                    },
                    "required": ["prompt"]
                }
            }
...
async fn call_tool(name: &str, args: Value) -> Result<Value, String> {
    match name {
        "list_music_models" => Ok(json!({ "models": list_music_models() })),
        "generate_music" => {
            let req: MusicGenRequest = serde_json::from_value(args)
                .map_err(|e| format!("Paramètres musicaux invalides: {e}"))?;
            let res = generate_music(req).await?;
            Ok(json!(res))
        }
        _ => Err(format!("Outil audio inconnu : {name}")),
    }
}
